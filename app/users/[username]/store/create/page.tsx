...
import { getProductForEdit, updateProduct } from "@/app/actions/updateProduct"
...
export default function CreateProductPage() {
  ...
  useEffect(() => {
    if (editId) {
      const fetchProduct = async () => {
        try {
          const product = await getProductForEdit(editId)
          if (product) {
            setFormData({
              ...
              license: product.license.toLowerCase() as any,
              productFiles: [], // We won't pre-fill files as it's complex, but we'll allow updates
            })
          } else {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      // 1. Upload/Get Cover Image
      if (formData.image) {
        toast.loading("Uploading cover image...", { id: "publish" })
        const imageRes = await uploadImage([formData.image])
        if (!imageRes) {
          toast.error("Failed to upload cover image", { id: "publish" })
          return
        }
        imageUrls = imageRes.map(f => f.url)
      ...
      // 2. Upload/Get Product Files
      if (formData.productFiles.length > 0) {
        toast.loading("Uploading product files...", { id: "publish" })
      ...
      toast.loading(editId ? "Updating product..." : "Publishing product...", { id: "publish" })
      ...
      const productData = {
        ...
        license: formData.license.toUpperCase() as any,
      ...
      toast.success(editId ? "Product updated!" : "Product published!", { id: "publish" })
      router.back()
      router.refresh()
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                <motion.div
                  ...
                  className="space-y-8"
                >
                  <h1 className="text-[2rem] font-bold text-slate-900 leading-tight">{editId ? "Edit Product" : "Create a Product"}</h1>
