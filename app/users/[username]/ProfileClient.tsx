...
import { Star, Pencil, Upload, ChevronRight, ImageIcon, PlusCircle, Loader2, X, Heart, MapPin, Globe, Link2, Plus, MessageSquare, ShieldAlert, ShieldOff, Clock, Flag } from "lucide-react"
...
export function ProfileClient({ user, services, portfolioItems, products, posts, reviews }: ProfileClientProps) {
    ...
    const handleEditItem = (item: any) => {
        if (item.type === 'SERVICE') {
            router.push(`/services/create?edit=${item.id}`)
        } else if (item.type === 'PRODUCT') {
            router.push(`/users/${user.username}/store/create?edit=${item.id}`)
        }
    }
                    ...
                    <div className="flex items-center justify-between border-b border-border/40">
                        ...
                        {user.isOwnProfile && (
                            <>
                                ...
                                {activeTab === "store" && (
                                    <Link href={`/users/${user.username}/store/create`}>
                                        <Button
                                            size="sm"
                                            className="rounded-full bg-foreground hover:bg-foreground/90 text-background font-medium h-9 px-4 text-sm flex items-center gap-1.5"
                                        >
                        ...
                        {portfolioItems && portfolioItems.length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-5">
                                {portfolioItems.map((item) => (
                                    <Link
                                        href={`/users/${user.username}/work/${item.id}`}
                                        key={item.id}
                                ...
                                {services.map((service, index) => (
                                    <div key={service.id} className="flex justify-center h-fit">
                                        ...
                                        <div className="w-[100%] h-full">
                                            <ExploreCard
                                                index={index}
                                                onEdit={user.isOwnProfile ? handleEditItem : undefined}
                                                item={{
                                                    id: service.id,
                                                    type: 'SERVICE',
                                                    title: service.title,
